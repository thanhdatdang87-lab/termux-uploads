...
if (GameState == "MainGame") then
    ...
    -- ===== ANTI KILL V13 - SMART LOCK HEIGHT (SIMPLIFIED) =====
    ...
    local function StartLockHeight()
        ...
        antiKillPlatform.Size = PLATFORM_SIZE
        ...
        antiKillConnection = game:GetService("RunService").Heartbeat:Connect(function()
            ...
            -- Tìm Ghost (đơn giản: Workspace:FindFirstChild("Ghost"))
            ...
            if Ghost then
                ...
                if ghostHRP then
                    -- Check điều kiện 1: Khoảng cách < range
                    local distance = (rootPart.Position - ghostHRP.Position).Magnitude
                    if distance < detectionRange then
                        -- Check điều kiện 2: Ghost đang hunt
            ...
            if shouldActivate then
                -- ✅ KÍCH HOẠT: Giữ lock height
                antiKillPlatform.Position = Vector3.new(rootPart.Position.X, lockTargetY - 3, rootPart.Position.Z)
                antiKillPlatform.CanCollide = true
            else
                -- ❌ TẮT: Thả player xuống
                if antiKillPlatform then
