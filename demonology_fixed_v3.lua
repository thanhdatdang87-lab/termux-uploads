...
if (GameState == "MainGame") then
    ...
    local function UpdateTracerLine(tracerData)
        ...
        line.From = centerPos
        -- Thêm offset 20px vào Y để tracer chỉ thẳng vào center item
        line.To = Vector2.new(screenPos.X, screenPos.Y + 20)
    ...
    local function CreateColorPickerDropdown(label, parent, position, objectType)
        local colorOptions = {
            {Color3.fromRGB(255, 0, 0), "Red"},
            {Color3.fromRGB(255, 100, 0), "Orange"},
            {Color3.fromRGB(255, 255, 0), "Yellow"},
            ...
            {Color3.fromRGB(128, 255, 128), "Light Green"},
            {Color3.fromRGB(128, 128, 255), "Light Blue"},
            {Color3.fromRGB(255, 255, 128), "Light Yellow"},
            {Color3.fromRGB(255, 128, 255), "Pink"}
        }
        
        local mainFrame = Instance.new("Frame")
        mainFrame.Name = label
        mainFrame.Size = UDim2.new(1, -20, 0, 30)
        mainFrame.Position = UDim2.new(0, 10, 0, position)
        ...
        labelText.Parent = mainFrame
        
        -- Current color display button
        local colorDisplay = Instance.new("Frame")
        colorDisplay.Name = "ColorDisplay"
        colorDisplay.Size = UDim2.new(0, 25, 0, 25)
        colorDisplay.Position = UDim2.new(1, -30, 0.5, -12)
        colorDisplay.BackgroundColor3 = BottomTracerConfig.ObjectTypes[objectType].Color
        colorDisplay.BorderSizePixel = 2
        colorDisplay.BorderColor3 = Color3.fromRGB(200, 200, 200)
        colorDisplay.Parent = mainFrame
        
        -- Dropdown frame (ẩn lúc đầu)
        local dropdownFrame = Instance.new("Frame")
        dropdownFrame.Name = "Dropdown"
        dropdownFrame.Size = UDim2.new(1, 0, 0, #colorOptions * 25 + 10)
        ...
        dropdownFrame.BorderColor3 = Color3.fromRGB(100, 100, 150)
        dropdownFrame.Visible = false
        dropdownFrame.ZIndex = 100
        dropdownFrame.Parent = mainFrame
        
        -- Scroll container for many colors
        local scrolling = Instance.new("ScrollingFrame")
        scrolling.Size = UDim2.new(1, 0, 1, 0)
        scrolling.BackgroundTransparency = 1
        scrolling.ScrollBarThickness = 8
        scrolling.CanvasSize = UDim2.new(0, 0, 0, #colorOptions * 25)
        ...
        for i, colorData in ipairs(colorOptions) do
            ...
            colorOption.Name = "Color_" .. i
            colorOption.Size = UDim2.new(1, 0, 0, 25)
            colorOption.Position = UDim2.new(0, 0, 0, (i - 1) * 25)
            colorOption.BackgroundColor3 = color
            colorOption.BorderSizePixel = 1
            colorOption.BorderColor3 = Color3.fromRGB(100, 100, 120)
            colorOption.Parent = scrolling
            ...
            clickArea.Text = ""
            clickArea.Parent = colorOption
            
            clickArea.MouseButton1Click:Connect(function()
                -- Cập nhật config
                BottomTracerConfig.ObjectTypes[objectType].Color = color
                
                -- Cập nhật display button
                colorDisplay.BackgroundColor3 = color
                
                -- Đóng dropdown
                dropdownFrame.Visible = false
                
                print("✅ [Tracer] " .. label .. " = " .. name)
            end)
            
            -- Hover effect
            colorOption.MouseEnter:Connect(function()
                colorOption.BorderColor3 = Color3.fromRGB(255, 255, 0)
                colorOption.BorderSizePixel = 2
            end)
        ...
        game:GetService("UserInputService").InputBegan:Connect(function(input, gameProcessed)
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                if dropdownFrame.Visible then
                    ...
                    local checkFrame = function(frame)
                        if not frame.Visible then return false end
                        local corner = frame.AbsolutePosition
                        local endPos = corner + frame.AbsoluteSize
                        return mousePos.X >= corner.X and mousePos.X <= endPos.X and
                               mousePos.Y >= corner.Y and mousePos.Y <= endPos.Y
                    end
                    
                    if checkFrame(dropdownFrame) or checkFrame(colorDisplay) then
                        isInside = true
                    end
    ...
    -- Tạo color picker dropdown
    CreateColorPickerDropdown("Ghost Tracer Color", UI["PlayerSettings"], 13, "Ghost")
    CreateColorPickerDropdown("Equipment Tracer Color", UI["PlayerSettings"], 40, "Equipment")
    CreateColorPickerDropdown("Cursed Tracer Color", UI["PlayerSettings"], 67, "CursedObject")
    CreateColorPickerDropdown("Evidence Tracer Color", UI["PlayerSettings"], 94, "Evidence")
