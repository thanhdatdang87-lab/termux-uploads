...
if (GameState == "MainGame") then
    ...
    -- ===== BOTTOM TRACER SYSTEM - STABLE VERSION =====
    ...
    local BottomTracerConfig = {
        ...
        ObjectTypes = {
            ...
            Evidence = {
                ...
                Thickness = 2,
                Transparency = 0.7,
                Patterns = {"Evidence", "EMF", "Thermometer"}
            },
            Default = {
                Enabled = true,
                Color = Color3.fromRGB(255, 255, 255),
    ...
    local function CreateTracerLine(object, objectType)
        local config = BottomTracerConfig.ObjectTypes[objectType] or BottomTracerConfig.ObjectTypes.Default
    ...
    local function UpdateTracerLine(tracerData)
        ...
        if not BottomTracerConfig.ESPEnabled[objectType] or not config.Enabled or not IsObjectOnScreen(object) then
            line.Visible = false
            return true
        end
    ...
    local function StartBottomTracerSystem()
        if TracerManager.UpdateConnection then
            TracerManager.UpdateConnection:Disconnect()
        end
        
        -- Limit update frequency (every 2 frames)
        local frameCount = 0
        TracerManager.UpdateConnection = game:GetService("RunService").RenderStepped:Connect(function()
            if not BottomTracerConfig.Enabled then return end
            
            frameCount = frameCount + 1
            if frameCount < 2 then return end  -- Skip every other frame
            frameCount = 0
            ...
            pcall(function()
                ...
                if BottomTracerConfig.ESPEnabled.Ghost then
                    local ghostModel = Workspace:FindFirstChild("Ghost")
                    if ghostModel then table.insert(objectsToTracer, ghostModel) end
                end
                ...
                for i = #removeIndices, 1, -1 do
                    table.remove(TracerManager.ActiveTracers, removeIndices[i])
                end
    ...
    local function StopBottomTracerSystem()
        if TracerManager.UpdateConnection then
            TracerManager.UpdateConnection:Disconnect()
            TracerManager.UpdateConnection = nil
        end
        
        for _, tracerData in pairs(TracerManager.ActiveTracers) do
            if tracerData.object then
                pcall(function() tracerData.object:Remove() end)
            end
        end
    ...
    tracerToggle['Activated']:Connect(function()
        ...
        if BottomTracerConfig.Enabled then
            StartBottomTracerSystem()
            print("✅ Bottom Tracer: ON")
        else
            StopBottomTracerSystem()
            print("❌ Bottom Tracer: OFF")
        end
    end)
    
    rainbowToggle['Activated']:Connect(function()
        BottomTracerConfig.RainbowMode = not BottomTracerConfig.RainbowMode
        print("🌈 Rainbow Tracer: " .. (BottomTracerConfig.RainbowMode and "ON" or "OFF"))
    end)
    ...
    LocalPlayer.CharacterAdded:Connect(function()
        if BottomTracerConfig.Enabled then
            task.wait(1)
            if not TracerManager.UpdateConnection or not TracerManager.UpdateConnection.Connected then
                StartBottomTracerSystem()
            end
        end
    end)
    ...
    game:GetService("RunService").Heartbeat:Connect(function()
        if not BottomTracerConfig.Enabled and TracerManager.UpdateConnection then
            StopBottomTracerSystem()
        end
    end)
