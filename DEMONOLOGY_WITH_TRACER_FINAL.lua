-- =====================================================
-- DEMONOLOGY EVIDENCE SUGGESTER + TRACER
-- File gốc + Tracer system từ DOORS
-- =====================================================
...
-- ====== TRACER SYSTEM (TỪ DOORS FILE) ======
local Camera = Workspace.CurrentCamera
local TracerEnabled = false
local TracerMode = "bottom" -- "bottom" hoặc "center"
local activeTracers = {}

local TracerColors = {
    Ghost = Color3.fromRGB(255, 0, 0),
    Equipment = Color3.fromRGB(0, 255, 0),
    CursedObject = Color3.fromRGB(128, 0, 255),
    Evidence = Color3.fromRGB(0, 255, 255)
}
...
local ColorOptions = {
    ...
    {Color3.fromRGB(255, 200, 100), "Light Orange"},
    {Color3.fromRGB(200, 200, 200), "Gray"},
    {Color3.fromRGB(150, 150, 150), "Dark Gray"},
    {Color3.fromRGB(0, 200, 150), "Teal"},
    {Color3.fromRGB(200, 0, 100), "Crimson"},
}

local function removeOldTracers()
    for target, line in pairs(activeTracers) do
        if line then
            pcall(function() line:Remove() end)
        end
        activeTracers[target] = nil
    end
end

local function createTracer(target, color, typeName)
    if not target or not target.Parent then return end
    if activeTracers[target] then
        activeTracers[target].Color = color
        return
    end
    ...
    activeTracers[target] = line
    
    task.spawn(function()
        while activeTracers[target] == line do
            if not TracerEnabled or not target or not target.Parent then break end
            
            local part = target:IsA("BasePart") and target or target.PrimaryPart or target:FindFirstChildWhichIsA("BasePart")
            if part then
        ...
        if activeTracers[target] == line then
            activeTracers[target] = nil
        end
        pcall(function() line:Remove() end)
    end)
...
local function updateTracers()
    ...
    if ghost then
        createTracer(ghost, TracerColors.Ghost, "Ghost")
    end
    ...
    if itemsFolder then
        for _, item in pairs(itemsFolder:GetChildren()) do
            createTracer(item, TracerColors.Equipment, "Equipment")
        end
    end
    ...
    if cursedFolder then
        for _, cursed in pairs(cursedFolder:GetChildren()) do
            createTracer(cursed, TracerColors.CursedObject, "Cursed")
        end
    end
    ...
    if evidenceFolder then
        for _, evidence in pairs(evidenceFolder:GetChildren()) do
            createTracer(evidence, TracerColors.Evidence, "Evidence")
        end
    end
end
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.T then
        ...
        print(TracerEnabled and "✅ Tracer ON" or "❌ Tracer OFF")
    end
    
    -- Y: Toggle mode
    if input.KeyCode == Enum.KeyCode.Y then
        TracerMode = TracerMode == "bottom" and "center" or "bottom"
        print("📍 Tracer mode: " .. TracerMode)
    end
    ...
    if input.KeyCode == Enum.KeyCode.O then
        ...
        for i, colorData in ipairs(ColorOptions) do
            if colorData[1] == TracerColors.Ghost then
                currentIndex = i
                break
            end
        end
        local nextIndex = currentIndex % #ColorOptions + 1
        local nextColor = ColorOptions[nextIndex]
        TracerColors.Ghost = nextColor[1]
        TracerColors.Equipment = nextColor[1]
        TracerColors.CursedObject = nextColor[1]
        TracerColors.Evidence = nextColor[1]
        print("🎨 Color: " .. nextColor[2])
    end
end)

print("\n=== TRACER CONTROLS ===")
print("T: Toggle Tracer")
print("Y: Toggle Bottom ↔ Center")
print("O: Cycle Colors (20+ options)")
print("======================\n")

-- PHẦN TỪ FILE GỐC (EVIDENCE SUGGESTER)
-- =====================================================
